...
namespace SenseGlove.Scripts.Tracking
{
    ...
    public class SG_HandPhysics : SG_HandComponent
    {
        ...
        protected override void CollectDebugComponents(out List<GameObject> objects, out List<MeshRenderer> renderers)
        {
            ...
            {
                SG_Util.CollectComponent(this.wristColliders[i].gameObject, renderers); //grab rendering components
            }
            for (int f = 0; f < this.fingerColliders.Length; f++)
            {
                SG_Util.CollectComponent(this.fingerColliders[f].gameObject, renderers); //grab rendering components
            }
        }
        ...
        protected override void CreateComponents()
        {
            ...
            this.handRigidbody = SG_Util.TryAddRB(this.gameObject, false, false);
            ...
            for (int i = 0; i < this.wristColliders.Length; i++)
            {
                ...
                SG_Util.TryRemoveComponent<SG_SimpleTracking>(wristColliders[i].gameObject);
            ...
            {
                fingerTracking[i] = SG_Util.TryAddComponent<SG_SimpleTracking>(fingerColliders[i].gameObject);
        ...
        public void UpdateRigidbody(SG_HandPose pose, float deltaTime, bool updateColliders = false)
        {
            ...
            {
                SG_Util.MoveRigidBody(this.handRigidbody, pose.wristPosition, pose.wristRotation, deltaTime,
