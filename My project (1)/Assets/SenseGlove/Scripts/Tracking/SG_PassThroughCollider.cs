...
namespace SenseGlove.Scripts.Tracking
{
	...
	public class SG_PassThroughCollider : SG_SimpleTracking
    {
        ...
        {
            return SG_Util.PrintContents(this.impassablesTouched, delim);
        }
        ...
        protected void TryAddCollider(Collider col)
        {
            if (!col.isTrigger)
            {
                bool added = SG_Util.SafelyAdd(col, this.impassablesTouched);
                if (added)
                {
                    this.UpdateDebugger();
                }
        ...
        {
            bool removed = SG_Util.SafelyRemove(col, this.impassablesTouched);
            if (removed)
            {
                this.UpdateDebugger();
            }
        ...
        {
            base.Awake();
            this.physicsBody = SG_Util.TryAddRB(this.gameObject, false, true); //make sure I have a rigidBody to trigger non-trigger colliders that don't have it.
