...
namespace SenseGlove.Scripts.Tracking
{
	...
	public class SG_FingerPassThrough : SG_HandComponent
	{
        ...
        protected override List<Collider> CollectPhysicsColliders()
        {
            ...
            {
                SG_Util.GetAllColliders(this.fingerColliders[f].gameObject, ref myColliders);
            }
            return myColliders;
        }
            ...
            for (int f = 0; f < 5; f++)
            {
                ...
                if (locked[f]) //we are locked, and we are passing though the maximum flexion.
                {
                    if (currentTotals[f] < lockedTotalFlex[f]) // < because in Unity, flexion is negative. This is what happens if your flexion is greater than where we first touched.
                    {
                        ...
                        SGCore.Kinematics.Vect3D[] iAngles = SG_Conversions.ToEuler(fingerAngles);
                        ...
                        jointPositions[f] = SG_Conversions.ToUnityPositions(iPositions);
                        jointRotations[f] = SG_Conversions.ToUnityQuaternions(iRotations);
