...
namespace SenseGlove.Scripts.Tracking
{
    ...
    public class SG_HandPose
    {
        ...
        {
            jointAngles = SG_Util.ArrayCopy(original.jointAngles);
            jointRotations = SG_Util.ArrayCopy(original.jointRotations);
            jointPositions = SG_Util.ArrayCopy(original.jointPositions);
            normalizedFlexion = SG_Util.ArrayCopy(original.normalizedFlexion);
        ...
        public SG_HandPose Mirror() //todo: make this part SGCore.HandPose as well.
        {
            ...
            for (int f = 0; f < jointRotations.Length; f++)
            {
                ...
                {
                    mirrorJoints[f][j] = SG_Conversions.MirrorZ(jointRotations[f][j]);
                }
            }
