...
namespace SenseGlove.Scripts.VR
{
    ...
    public class SG_VR_RoomSetup : MonoBehaviour
	{
        ...
        public void CalculateRigLocation(Transform vrRig, Transform headLocation, Transform headTarget)
        {
            if (matchRotation)
            {
                ...
                float yTarget = SG_Util.NormalizeAngle(headTarget.rotation.eulerAngles.y);
                float yRig = SG_Util.NormalizeAngle(vrRig.rotation.eulerAngles.y);
                float yCam = SG_Util.NormalizeAngle(headLocation.rotation.eulerAngles.y);
