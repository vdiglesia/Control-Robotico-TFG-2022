...
using SenseGlove.Scripts.Devices;
	...
	public class SG_WireFrame : MonoBehaviour
	{
        ...
        public void GenerateGlove(SGCore.SG.SG_GloveInfo gloveModel)
        {
            if (gloveModel != null)
            {
                ...
                Vector3[][] gloveLengths = SG_Conversions.ToUnityPositions(gloveModel.GloveLengths, true);
                int x = 0, y = 1, z = 2;
        ...
        public void UpdateGlove(SGCore.SG.SG_GlovePose glovePose)
        {
            ...
            {
                Quaternion[][] angles = SG_Conversions.ToUnityQuaternions(glovePose.JointRotations);
                Vector3[][] positions = SG_Conversions.ToUnityPositions(glovePose.JointPositions);
                if (gloveJoints != null)
                {
                    for (int f = 0; f < gloveJoints.Length; f++)
                    {
