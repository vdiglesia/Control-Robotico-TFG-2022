...
namespace SenseGlove.Scripts.Util
{
    ...
    public static class SG_Util
    {
        ...
        public static Vector3 Map(float x, float x0, float x1, Vector3 y0, Vector3 y1)
        {
            return new Vector3
            (
                SG_Util.Map(x, x0, x1, y0.x, y1.x),
                SG_Util.Map(x, x0, x1, y0.y, y1.y),
                SG_Util.Map(x, x0, x1, y0.z, y1.z)
            );
        }
        ...
        {
            if (keyCode != KeyCode.None) { SG_Util.SetButtonText(button, baseTxt + "\r\n[" + keyCode + "]"); }
            else { SG_Util.SetButtonText(button, baseTxt); }
        }
        ...
        public static void GetCurrentBaseLocation(Transform objToCalculate, Transform originalParent, Vector3 basePosition_original, Quaternion baseRotation_original, out Vector3 basePosition_current, out Quaternion baseRotation_current)
        {
            ...
            {
                SG_Util.CalculateTargetLocation(originalParent, basePosition_original, baseRotation_original, out basePosition_current, out baseRotation_current);
            }
        }
