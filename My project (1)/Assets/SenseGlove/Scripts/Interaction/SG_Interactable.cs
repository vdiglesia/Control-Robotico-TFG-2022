...
namespace SenseGlove.Scripts.Interaction
{
    ...
    public class GrabArguments : HoverArguments
    {
        ...
        {
			SG_Util.CalculateTargetLocation(this.GrabScript.realGrabRefrence, GrabOffset_Position, GrabOffset_Rotation, out objTargetPos, out objTargetRot);
        }
        ...
        {
			SG_Util.CalculateRefrenceLocation(obj.position, obj.rotation, GrabOffset_Position, GrabOffset_Rotation, out objTargetPos, out objTargetRot);
        }
    ...
    public class SG_Interactable : MonoBehaviour
	{
        ...
        public void LinkGrabPoint(SG_GrabPoint point)
        {
            if (point != this)
            {
                SG_Util.SafelyAdd(point, linkedGrabPoints);
            }
            else
            {
        ...
        {
            SG_Util.SafelyRemove(point, linkedGrabPoints);
        }
        ...
        protected virtual List<Collider> CollectPhysicsColliders()
        {
            ...
            SG_Util.GetAllColliders(this.gameObject, ref res, true);
            return res;
        }
		...
		public virtual GrabArguments GenerateGrabArgs(SG_GrabScript grabScript)
        {
			...
			SG_Util.CalculateOffsets(myTransf, grabScript.virtualGrabRefrence, out posOffset, out rotOffset); //virtualGrabReference. That's where we need to go.
