...
namespace SenseGlove.Scripts.Interaction
{
    ...
    public class SG_Drawer : SG_Grabable
    {
        ...
        protected void CalculateSliderValue()
        {
            ...
            this.drawer_slideValue = SG_Util.Map(drawerDist, push, pull, 0, 1);  //no need to clamp, the drawerDist is locked.
        }

        /// <summary> Set the current location of the drawer as "0" for the drawerDistance. </summary>
        public void RecalculateBaseLocation()
        {
            SG_Util.CalculateBaseLocation(this.MyTransform, out localStartPos, out localStartRot);
        }
        ...
        {
            SG_Util.GetCurrentBaseLocation(this.MyTransform, localStartPos, localStartRot, out basePos, out baseRot);
        }
        ...
        public Vector3 DrawerDirection
        {
            get
            {
                Vector3 localAxis = SG_Util.GetAxis( (Util.SG_Util.MoveAxis) this.moveAxis ); //I can cast to the Utils moveAxis because the also use XYZ at the start
        ...
        public static void CalculateDrawerTarget(SG_Drawer drawer, Vector3 nextPositon, out Vector3 targetPos, out Quaternion targetRot, out float drawerDist)
        {
           // Calculate the ference point for this drawer
            ...
            Vector3 projectedPos = SG_Util.ProjectOnTransform(nextPositon, basePos, targetRot);
        ...
        public void SetDrawerAt(float slideValue01)
        {
            ...
            if (slideValue01 != this.drawer_slideValue) //optimization: Don't change if you're already there.
            {
                ...
                float push = -Mathf.Abs(this.pushDistance);
                float pull = Mathf.Abs(this.pullDistance);
                float localDisplacement = SG_Util.Map(slideValue01, 0, 1, push, pull);  //no need to clamp, the drawerDist is locked.
