...
using SenseGlove.Scripts.Feedback;
    ...
    public class SG_HapticGlove : MonoBehaviour, IHandPoseProvider, IHandFeedbackDevice
    {
        ...
        public virtual bool GetIMURotation(out Quaternion imuRotation)
        {
            if (this.lastGlove != null)
            {
                ...
                {
                    imuRotation = SG_Conversions.ToUnityQuaternion(Q);
                    return true;
                }
            }
        ...
        public virtual bool GetGloveLocation(Transform trackedObject, SGCore.PosTrackingHardware hardware, out Vector3 glovePos, out Quaternion gloveRot)
        {
            if (this.lastGlove != null)
            {
                SGCore.Kinematics.Vect3D trackedPos = SG_Conversions.ToPosition(trackedObject.position, true);
                SGCore.Kinematics.Quat trackedRot = SG_Conversions.ToQuaternion(trackedObject.rotation);
                ...
                glovePos = SG_Conversions.ToUnityPosition(wPos);
                gloveRot = SG_Conversions.ToUnityQuaternion(wRot);
                return true;
            }
        ...
        public virtual bool GetWristLocation(Transform trackedObject, SGCore.PosTrackingHardware hardware, out Vector3 wristPos, out Quaternion wristRot)
        {
            ...
            wristPos = SG_Conversions.ToUnityPosition(wPos);
            wristRot = SG_Conversions.ToUnityQuaternion(wRot);
            return true;
        }
        ...
        {
            SgConnections.SetupConnections();
            UpdateDeviceType();
