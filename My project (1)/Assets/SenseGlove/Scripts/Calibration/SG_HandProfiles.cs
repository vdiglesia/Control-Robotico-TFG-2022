...
namespace SenseGlove.Scripts.Calibration
{
	...
	public class SG_HandProfiles
	{
		...
		public static SGCore.HandProfile RightHandProfile
		{
			...
			set
			{
				...
				rightProfile = value;
				if (FileIO.SaveTxtFile(ProfileDirectory, rightHandFile, new string[] { rightProfile.Serialize() }))
				{
		...
		public static SGCore.HandProfile LeftHandProfile
        {
            ...
            set
            {
				if (value == null) { throw new System.ArgumentNullException(); }
				leftProfile = value;
				if (FileIO.SaveTxtFile(ProfileDirectory, leftHandFile, new string[] { leftProfile.Serialize() }))
				{
					triedLoading = true; //we don't need to load again, as the latest is in out memory
				}
				else
        ...
        {
			string[] lines;
			if (FileIO.ReadTxtFile(filePath, out lines) && lines.Length > 0)
            {
                try
                {
		...
		public static bool SaveLastRange(SGCore.Calibration.SensorRange currentRange, SGCore.HapticGlove forGlove)
		{
			if (forGlove != null)
			{
				...
				return FileIO.SaveTxtFile(ProfileDirectory + rangeDir, name, new string[] { currentRange.Serialize() });
			}
			return false;
		}
		...
		public static bool LoadLastRange(SGCore.HapticGlove forGlove, out SGCore.Calibration.SensorRange lastRange)
		{
			if (forGlove != null)
			{
				string name = forGlove.GetDeviceID() + ".txt";
				string[] lines;
				if (FileIO.ReadTxtFile(ProfileDirectory + rangeDir + name, out lines) && lines.Length > 0 && lines[0].Length > 0)
				{
