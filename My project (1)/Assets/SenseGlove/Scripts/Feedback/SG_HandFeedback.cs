	...
	public class SG_HandFeedback : SG_HandComponent
	{
        ...
        protected override void LinkToHand(SG_TrackedHand newHand, bool firstLink)
        {
            ...
            for (int f = 0; f < fingerFeedbackScripts.Length && f < 5; f++)
            {
                ...
                Rigidbody fbBody = SG_Util.TryAddComponent<Rigidbody>(fingerFeedbackScripts[f].gameObject);
        ...
        protected override List<Collider> CollectPhysicsColliders()
        {
            ...
            {
                SG_Util.GetAllColliders(this.fingerFeedbackScripts[f].gameObject, ref myColliders);
            }
            return myColliders;
        }
        ...
        protected override void CollectDebugComponents(out List<GameObject> objects, out List<MeshRenderer> renderers)
        {
            ...
            for (int f = 0; f < this.fingerFeedbackScripts.Length; f++)
            {
                SG_Util.CollectComponent(this.fingerFeedbackScripts[f].gameObject, renderers); //deleteable renderer of the GameObject
